import React, { useState, useEffect } from "react";
import axios from "axios";
import { useParams } from "react-router-dom";
import { Helmet } from "react-helmet";

import "../../App.css";

import TopHeader from "../../components/TopHeader";

import SerieGenres from "../../components/UI/Serie_Genres";

import SerieCard from "../../components/UI/Serie_Card";

import SerieSearch from "../../components/UI/Serie_Search";

import LoadingSpinner from "../../components/Loading_Spinner";

function Series_By_Genre() {
  const [series, setSeries] = useState([]);
  const [page, setPage] = useState(1);
  const { genreId } = useParams("");
  const [loading, setLoading] = useState(false);
  const [pageName, setPageName] = useState("");

  useEffect(() => {
    // setLoading(false);
    const getAllMovies = async () => {
      try {
        const response = await axios.get(
          `https://api.themoviedb.org/3/discover/tv?api_key=f2552b7af8b6069509ae2de03f1c8c85&with_genres=${genreId}&page=${page}`
        );

        setSeries((prevTrendingMovies) => {
          if (page === 1) {
            return response.data.results.slice(0, 20);
          } else {
            return [...prevTrendingMovies, ...response.data.results];
          }
        });
        setLoading(true);
      } catch (err) {
        console.log(
          "error getting the trending movies from the trending movies page"
        );
        setLoading(false);
      }
    };

    getAllMovies();
  }, [page, genreId]);

  useEffect(() => {
    const getGenres = async () => {
      try {
        const response = await axios.get(
          "https://api.themoviedb.org/3/genre/tv/list?api_key=f2552b7af8b6069509ae2de03f1c8c85"
        );

        const genre = response.data.genres.filter(
          (genre) => genre.id === +genreId
        );

        setPageName(genre[0].name);
      } catch (err) {
        console.error("Error fetching movie genres", err);
      }
    };

    getGenres();
  }, [genreId]);

  return (
    <React.Fragment>
      {loading === false && <LoadingSpinner />}
      <Helmet>
        <title>{pageName}</title>
      </Helmet>

      {loading === true && (
        <React.Fragment>
          <TopHeader />
          <SerieSearch />
          <div className="cards-main-container">
            <div className="card-btn-container">
              <div className="cards-container">
                {series.map((movie) => (
                  <SerieCard
                    key={movie.id}
                    poster_path={movie.poster_path}
                    name={movie.name}
                    id={movie.id}
                  />
                ))}
              </div>

              <button
                onClick={() => {
                  setPage((prevPage) => prevPage + 1);
                }}
                className="load-more-btn"
              >
                Load more
              </button>
            </div>

            <SerieGenres genre={genreId} />
          </div>
        </React.Fragment>
      )}
    </React.Fragment>
  );
}

export default Series_By_Genre;
